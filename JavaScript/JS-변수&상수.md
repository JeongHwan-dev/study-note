...
습관적으로 값이 전혀 변하지 않는 데도 `let`으로 선언하여 변수로 만들어서 다루는 것은 굉장히 나쁜 습관입니다. 그러므로 코드 자체에서 한 번도 값이 세팅되고 나서 변하지 않는다면 항상 상수 `const`로 선언하고 값이 변한다고 할 때만 `let`으로 변수를 만들어서 사용해야 합니다.
